...
def setup_hopp(
    ...
    save_plots=False,
    output_dir="./output/",
    hopp_site: SiteInfo | None = None,
):
    if isinstance(output_dir, str):
        output_dir = Path(output_dir).resolve()

    if hopp_site is None:
        ...
        if hopp_config["site"]["wind"]:
            if hopp_config["technologies"]["wind"]["model_name"] == "floris":
                if design_scenario["wind_location"] == "offshore":
                    ...
                    # remove things from turbine_config file that can't be used in FLORIS and
                    # set the turbine info in the floris config file
                ...
                msg = (
                    f'Wind model `{hopp_config["technologies"]["wind"]["model_name"]}` not'
                    'available Please choose one of ["floris", "pysam"]'
                )
    ...
    if "wave" in hopp_config_internal["technologies"].keys() and ("site" in hopp_config.keys()):
        wave_cost_dict = hopp_config_internal["technologies"]["wave"].pop("cost_inputs")

    if "battery" in hopp_config_internal["technologies"].keys() and ("site" in hopp_config.keys()):
        hopp_config_internal["site"].update({"desired_schedule": hopp_site.desired_schedule})

    if "site" not in hopp_config_internal:
        hopp_config_internal["site"] = {"data": {}}
